...
public:
    ...
    const CHANNEL_PARAM_DATA_TDS& get_channel_params(FS_HIGH_CH_TDE FS_HI, R1_RATE_TDE R1, R2_RATE_TDE R2, R3_RATE_TDE R3);
...
{
    if (current_nA>MAX_LEADOFF_DETECTION_CURRENT)
    {
        current_nA=MAX_LEADOFF_DETECTION_CURRENT;
    }
...
inline uint32_t ADS1293_LIB::get_AC_leadoff_frequency(void)
{
    ...
    uint32_t K=1;
    if (R_LOD_AC_CN.S.ACDIV_FACTOR==true)
    {
        K=16;
    }
...
inline void ADS1293_LIB::set_wilson_reference_control(RG_WLS_REFCNTRL_TDE ref_control)
{
    ...
    if (ref_control==RG_WLS_REFCNTRL_GOLDINT)
    {
        R_WILSON_CN.S.GOLDINT=1;
    }
    if (ref_control==RG_WLS_REFCNTRL_WILSONINT)
    {
        R_WILSON_CN.S.WILSONINT=1;
    }
...
inline RG_WLS_REFCNTRL_TDE ADS1293_LIB::get_wilson_reference_control(void)
{
    ...
    if (R_WILSON_CN.S.WILSONINT==0 && R_WILSON_CN.S.GOLDINT==0)
    {
        return RG_WLS_REFCNTRL_NOT_SET;
    }
    if (R_WILSON_CN.S.WILSONINT==1 && R_WILSON_CN.S.GOLDINT==0)
    {
        return RG_WLS_REFCNTRL_WILSONINT;
    }
    if (R_WILSON_CN.S.WILSONINT==0 && R_WILSON_CN.S.GOLDINT==1)
    {
        return RG_WLS_REFCNTRL_GOLDINT;
    }
...
{
    if(clk_frequency==FS_HIGH_CH_102400HZ)
    {
        return 102400;
    }
    if(clk_frequency==FS_HIGH_CH_204800HZ)
    {
        return 204800;
    }
}
...
{
    if (rate==R1_RATE_DOUBLE_R1_2)
    {
        return 2;
    }
    if (rate==R1_RATE_STANDART_R1_4)
    {
        return 4;
    }
    return 1;
...
inline const CHANNEL_PARAM_DATA_TDS& ADS1293::ADS1293_LIB::get_channel_params(FS_HIGH_CH_TDE FS_HI, R1_RATE_TDE R1, R2_RATE_TDE R2, R3_RATE_TDE R3)
{
    uint32_t base_freq=0,R1_ind=0,R2_ind=0,R3_ind=0;

    if (FS_HI==FS_HIGH_CH_204800HZ) base_freq=1;
    if (R1==R1_RATE_DOUBLE_R1_2) R1_ind=1;
    ...
    return ch_param_table[base_freq][R1_ind][R2_ind][R3_ind];
}
