#!/usr/bin/env python3
"""
ICG-ECG 1-Hour Drift Analysis Test - V2 for Updated Firmware
...
Updated for new firmware in SOURCE CODE(MAX,ADC,LED,POWER) folder.

Test Configuration:
- ICG: 400 Hz output, 50kHz stimulation, 640uA current
- ECG: R1=4, R2=8, R3=16 (400 Hz)
- Duration: 1 hour (3600 seconds)
- Data request interval: Configurable (default: 0.2 seconds)

Key Changes from V1:
- ICG uses stimulate_frequency in Hz (not index)
- ICG uses stimulate_current as string ("640uA")
- ICG filters use "BYPASS" string (not 0)
- ECG has new R1_rate parameter
- Uses "poweroff" command instead of power_enable=false
- Handles firmware state machine responses during ICG configuration

Usage:
    python3 test_icg_ecg_drift_1hour_v2.py [host] [threshold_ms] [data_interval_s]
...
Examples:
    python3 test_icg_ecg_drift_1hour_v2.py
    python3 test_icg_ecg_drift_1hour_v2.py localhost 50 1.0
    python3 test_icg_ecg_drift_1hour_v2.py 192.168.1.100 10 0.5
...
class DeviceClient:
    ...
    def receive_response(self, timeout: float = 1.0) -> Optional[dict]:
        """Receive a response without sending a command (for state machine messages)"""
        try:
...
class DriftTester:
    ...
    def __init__(self, host: str = "localhost", sync_threshold_ms: float = 50.0,
        ...
        # ICG configuration - UPDATED FOR NEW FIRMWARE
        # New firmware uses actual Hz values and string parameters
        self.icg_config = {
            "type": "settings",
            "measure_enable": True,
            "measure_frequency": 400,           # Output data rate in Hz
            "stimulate_frequency": 50000,       # 50kHz stimulation (was index 8)
            "stimulate_current": "640uA",       # Current as string (was index 3-4)
            "out_HP_filter": "BYPASS",          # String instead of int
            "out_LP_filter": "BYPASS",          # String instead of int
            "input_HP_filter": "BYPASS"         # NEW parameter
        }

        # ECG configuration - UPDATED FOR NEW FIRMWARE
        # Added R1_rate parameter, removed power_enable
        # Formula: ODR = 204800 / (R1 * R2 * R3)
        # For 400 Hz: 204800 / (4 * 8 * 16) = 400 Hz
        self.ecg_config = {
            "type": "settings",
            "enable_conversion": True,
            "R1_rate": 4,                       # NEW parameter (2 or 4)
            "R2_rate": 8,                       # Changed from 4 to 8 for 400Hz
            "R3_rate": 16                       # Same as before
        }
    ...
    def stop_icg(self) -> bool:
        """Stop ICG measurement and power down - UPDATED FOR NEW FIRMWARE"""
        # First disable measurement
        stop_config = {"type": "settings", "measure_enable": False}
        self.icg_client.send_command(stop_config)
        time.sleep(0.1)

        # Then power off using new poweroff command
        response = self.icg_client.send_command({"type": "poweroff"})
        return response is not None

    def stop_ecg(self) -> bool:
        """Stop ECG conversion and power down - UPDATED FOR NEW FIRMWARE"""
        # First disable conversion
        stop_config = {"type": "settings", "enable_conversion": False}
        self.ecg_client.send_command(stop_config)
        time.sleep(0.1)

        # Then power off using new poweroff command
        response = self.ecg_client.send_command({"type": "poweroff"})
        return response is not None
    ...
    def wait_for_icg_ready(self, timeout: float = 60.0) -> bool:
        """Wait for ICG state machine to complete startup sequence

        New firmware sends multiple state messages:
        - meas_state: pre_measuring
        - meas_state: pre_measure_end
        - meas_state: calibrating
        - calib_data: {...}
        - meas_state: calibrate_end
        - meas_state: start_measuring
        - actual_settings: {...}
        """
        print("    Waiting for ICG state machine...")
        start_time = time.time()

        while time.time() - start_time < timeout:
            response = self.icg_client.receive_response(timeout=2.0)
            if response:
                resp_type = response.get('type', '')

                if resp_type == 'actual_settings':
                    print(f"    ICG ready: actual_settings received")
                    return True
                elif resp_type == 'meas_state':
                    state = response.get('state', 'unknown')
                    print(f"    ICG state: {state}")
                elif resp_type == 'calib_data':
                    print(f"    ICG calibration data received")
                elif resp_type == 'error JSON' or resp_type == 'error settings JSON':
                    print(f"    ICG error: {resp_type}")
                    return False
            else:
                # No response, check if we've timed out
                if time.time() - start_time > timeout:
                    break

        print(f"    ICG timeout after {timeout}s")
        return False

    def configure_devices(self) -> bool:
        """Configure both devices with test settings - UPDATED FOR NEW FIRMWARE"""

        # Configure ECG first (simpler, no state machine)
        print("  Configuring ECG...")
        print(f"    R1={self.ecg_config['R1_rate']}, R2={self.ecg_config['R2_rate']}, "
              f"R3={self.ecg_config['R3_rate']}")
        print(f"    Expected ODR: {204800 // (self.ecg_config['R1_rate'] * self.ecg_config['R2_rate'] * self.ecg_config['R3_rate'])} Hz")
        ...
        if ecg_response.get('type') == 'actual_settings':
            print(f"    ECG configured successfully")
        else:
            print(f"    ECG response: {ecg_response.get('type', 'unknown')}")

        # Configure ICG (has state machine with calibration)
        print("  Configuring ICG...")
        print(f"    measure_freq={self.icg_config['measure_frequency']}Hz, "
              f"stim_freq={self.icg_config['stimulate_frequency']}Hz, "
              f"current={self.icg_config['stimulate_current']}")

        icg_response = self.icg_client.send_command(self.icg_config)
        if not icg_response:
            print("  Failed to send ICG config")
            return False

        # Handle ICG state machine responses
        resp_type = icg_response.get('type', '')
        if resp_type == 'meas_state':
            # State machine started, wait for it to complete
            state = icg_response.get('state', '')
            print(f"    ICG state: {state}")
            if not self.wait_for_icg_ready(timeout=60.0):
                print("  ICG did not reach ready state")
                return False
        elif resp_type == 'actual_settings':
            print(f"    ICG configured immediately (no calibration)")
        elif resp_type == 'error JSON' or resp_type == 'error settings JSON':
            print(f"  ICG configuration error: {resp_type}")
            return False
        else:
            print(f"    ICG response: {resp_type}")
        ...
        while time.time() - start_time < duration:
            ...
            if icg_data and 'data' in icg_data:
                try:
                    ...
                    # Count sync markers (magic number: -999990000)
            ...
            elif icg_data and icg_data.get('type') == 'no_measure':
                # ICG is not measuring
            ...
            if ecg_data and 'data' in ecg_data:
                try:
                    ...
                    # Count sync markers (magic number: -99999)
            ...
            if icg_sync_count > 1:
                ...
                print(f"    ANOMALY: Multiple ICG syncs in request {self.request_counter}: {icg_sync_numbers}")
            ...
            if ecg_sync_count > 1:
                ...
                print(f"    ANOMALY: Multiple ECG syncs in request {self.request_counter}: {ecg_sync_numbers}")
    ...
    def generate_anomaly_report(self):
        ...
        report = []
        report.append("="*80)
        report.append("ICG-ECG DRIFT ANALYSIS - ANOMALY REPORT (V2 - New Firmware)")
        report.append("="*80)
        report.append("")
        ...
        report.append("")
        report.append("ICG Configuration:")
        report.append(f"  stimulate_frequency: {self.icg_config['stimulate_frequency']} Hz")
        report.append(f"  stimulate_current: {self.icg_config['stimulate_current']}")
        report.append(f"  measure_frequency: {self.icg_config['measure_frequency']} Hz")
        report.append("")
        report.append("ECG Configuration:")
        report.append(f"  R1_rate: {self.ecg_config['R1_rate']}")
        report.append(f"  R2_rate: {self.ecg_config['R2_rate']}")
        report.append(f"  R3_rate: {self.ecg_config['R3_rate']}")
        report.append(f"  Calculated ODR: {204800 // (self.ecg_config['R1_rate'] * self.ecg_config['R2_rate'] * self.ecg_config['R3_rate'])} Hz")
        report.append("")
        ...
        if self.anomalies['multiple_syncs_icg'] or self.anomalies['multiple_syncs_ecg']:
            ...
            report.append("  -> Increase data polling frequency or reduce sampling rate")
    ...
    def save_json_results(self, icg_syncs_count: int, ecg_syncs_count: int) -> str:
        ...
        output = {
            'test_name': 'ICG-ECG 1-Hour Drift Analysis V2 (New Firmware)',
    ...
    def run_test(self) -> bool:
        ...
        print("\n" + "="*80)
        print("ICG-ECG 1-HOUR DRIFT ANALYSIS TEST V2 (NEW FIRMWARE)")
        print("="*80)
        ...
        print("")
        print("ICG Config: stim_freq={0}Hz, current={1}, measure_freq={2}Hz".format(
            self.icg_config['stimulate_frequency'],
            self.icg_config['stimulate_current'],
            self.icg_config['measure_frequency']))
        print("ECG Config: R1={0}, R2={1}, R3={2} -> ODR={3}Hz".format(
            self.ecg_config['R1_rate'],
            self.ecg_config['R2_rate'],
            self.ecg_config['R3_rate'],
            204800 // (self.ecg_config['R1_rate'] * self.ecg_config['R2_rate'] * self.ecg_config['R3_rate'])))
        print("="*80)
        ...
        try:
            ...
            print("  Waiting 3 seconds for devices to stabilize...")
            time.sleep(3)

            # Flush any data accumulated during stabilization
            self.flush_buffers(num_requests=3)
