#!/bin/bash
# Wrapper script to run SPI_DEV_servise with date-based log rotation
...
archive_old_logs() {
    ...
    local service_name="SPI_DEV_servise"
...
LOG_FILE="$LOG_DIR/SPI_DEV_servise-$(date +%Y-%m-%d).log"

# Ensure log directory exists
mkdir -p "$LOG_DIR"

# Log startup to syslog
logger "SPI_DEV_servise wrapper starting - logging to $LOG_FILE"

# Log startup to dated log file
echo "$(date '+%Y-%m-%d %H:%M:%S') - SPI_DEV_servise wrapper starting (PID: $$)" >> "$LOG_FILE"
...
exec /home/pi/spi-service "$@" >> "$LOG_FILE" 2>&1
