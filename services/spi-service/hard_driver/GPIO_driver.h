...
public:
    ...
    bool set_GPIO_direct(VT_GPIO_DIRECT_TDE direct,VT_GPIO_STATE_TDE init_state=VT_GPIO_UNSET)
    {
        ...
        if (_direct==direct && _line) {return true;} // not need change direct
        ...
        if (_line)
        {
            ...
            if (direct==VT_GPIO_OUTPUT)
            {
                ...
                if (init_state==VT_GPIO_SET) {init_state_value=1;}
    ...
    VT_GPIO_STATE_TDE get_GPIO_state(void)
    {
        ...
        if (value==0) {return VT_GPIO_UNSET;}
        if (value==1) {return VT_GPIO_SET;}
